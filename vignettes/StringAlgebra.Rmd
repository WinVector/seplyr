...
refering to names, quoted strings, and general expression terms.  It then "dequotes"
all of the variables refering to quoted strings and  "dereferences" variables 
thought to be refering to names.  The entire expression is then return as a single string.
...
alone as it was not refering to a string (and hence can not be a name, unbound or free variables
are also left alone).  So as we see the substitution performed does depend on what values
...
`seplyr` also provided an number of `seplyr::*_nse()` convience forms wrapping all of these steps into
