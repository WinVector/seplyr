...
Details on [`%.>%`](https://winvector.github.io/wrapr/reference/grapes-.-greater-than-grapes.html) (dot pipe) and [`:=`](https://winvector.github.io/wrapr/reference/named_map_builder.html) (named map builder) can be found [here](https://winvector.github.io/wrapr/articles/dot_pipe.html) and [here](https://winvector.github.io/seplyr/articles/named_map_builder.html) respectively.


`seplyr` also provided an number of `seplyr::*_nse()` convenience forms wrapping all of these steps into one operation.  For example:
...
The main rule when working with `seplyr` is: as the purpose of the `seplyr` string system is to pull-off quotes and de-reference, you need to remember to add enough extra quotation marks to prevent this where you do not want it.
