...
We hope this makes it easy to translate one-off analyses into re-usable scripts by incrementally replacing known variable names with parametric versions.

In contrast `dplyr::rename_at()` has its own notation that seem to not imitate `dplyr::rename()`.  The same operation using `dplyr::rename_at()` is something like the following.
...
This is sensible, but is very different than `dplyr::rename()`.
