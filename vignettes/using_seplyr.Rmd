...
This inessential (or conquerable) difficulty is largely a due to the strong preference for [non-standard evaluation interfaces](http://adv-r.had.co.nz/Computing-on-the-language.html) (that is interfaces that capture and inspect un-evaluated expressions from their calling interface) in the design `dplyr`.  The current suggested tooling to work around `dplyr`'s non-standard interfaces is a system called alternately `rlang` (a name which is perhaps a bit overly ambitious as `rlang` is not the `R` language) or tidyeval (a name which is perhaps a bit derogatory, incorrectly labeling `R` itself as the "non-tidy" dialect relative to tidyeval).
...
one chooses an existing column name as the name of a column control variable.
...
therefore can be held responsible for avoiding column name to variable coincidences. Or more realistically: the user is working in
...
However, because the `dplyr` interface does not insist on the "`!!`" notation, the "`!!`" notation will not always be remembered.

The `seplyr::select_se()` interface is intentionally simpler and more reliable: it always looks for columns using the strings (not variable names) given.
