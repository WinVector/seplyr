...
`seplyr` methods are short and have examples in their help, so always try both help and printing the method (for example: `help(select_se)` and `print(select_se)`).
